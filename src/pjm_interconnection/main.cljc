(ns pjm_interconnection.main
  "Kotodama WASM entrypoint for the pjm_interconnection clean-room actor (L4) — Clojure port.
  ...
  facts (`pjm_interconnection.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "pjm_interconnection")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"pjm_interconnection.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "pjm_interconnection-compat" :tier tier :entities entities} 200])
...
(defn start! [] :pjm_interconnection-compat/ready)
