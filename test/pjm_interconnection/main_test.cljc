(ns pjm_interconnection.main-test
  "Contract + behavioral test for the pjm_interconnection-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "pjm_interconnection-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'pjm_interconnection.main-test)]
